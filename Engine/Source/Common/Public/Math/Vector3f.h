...
struct Vector4f;
struct Matrix4f;
struct FROSTY_COMMON_API Vector3f
{
	float X, Y, Z;
	...
	Vector3f& Abs();
	Vector3f& Lerp(const Vector3f& destination, float time);
	...
	inline friend FROSTY_COMMON_API Vector3f operator+(Vector3f left, const Vector3f& right) { return left.Add(right); }
	inline friend FROSTY_COMMON_API Vector3f operator-(Vector3f left, const Vector3f& right) { return left.Subtract(right); }
	inline friend FROSTY_COMMON_API Vector3f operator*(Vector3f left, const Vector3f& right) { return left.Multiply(right); }
	inline friend FROSTY_COMMON_API Vector3f operator/(Vector3f left, const Vector3f& right) { return left.Divide(right); }
	 
	inline friend FROSTY_COMMON_API Vector3f operator+(Vector3f left, float right) { return left.Add(right); }
	inline friend FROSTY_COMMON_API Vector3f operator-(Vector3f left, float right) { return left.Subtract(right); }
	inline friend FROSTY_COMMON_API Vector3f operator*(Vector3f left, float right) { return left.Multiply(right); }
	inline friend FROSTY_COMMON_API Vector3f operator/(Vector3f left, float right) { return left.Divide(right); }

	inline Vector3f& operator-() { return this->Negative(); }
	...
	friend FROSTY_COMMON_API std::ostream& operator<<(std::ostream& stream, const Vector3f& right);
