...
Vector3f Vector3f::Multiply(const Matrix4f& matrix) const
{
	return Vector3f
	(
		matrix.Rows[0].X * this->X + matrix.Rows[0].Y * this->Y + matrix.Rows[0].Z * this->Z + matrix.Rows[0].W,
		matrix.Rows[1].X * this->X + matrix.Rows[1].Y * this->Y + matrix.Rows[1].Z * this->Z + matrix.Rows[1].W,
		matrix.Rows[2].X * this->X + matrix.Rows[2].Y * this->Y + matrix.Rows[2].Z * this->Z + matrix.Rows[2].W
	);
}
...
{
	float sineAngle = static_cast<float>(std::sin(-angle));
	float cosineAngle = static_cast<float>(std::cos(-angle));

	Vector3f result = Cross(axis * sineAngle) + ((*this) * cosineAngle) + axis * (Dot(axis * 1 - cosineAngle));
	this->X = result.X;
	this->Y = result.Y;
...
Vector3f& Vector3f::Lerp(const Vector3f& destination, float time)
{
	Vector3f result = (destination - (*this) * time + (*this));
	this->X = result.X;
	this->Y = result.Y;
