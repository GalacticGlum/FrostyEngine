...
Matrix4f Matrix4f::Perspective(float fov, float aspectRatio, float zNear, float zFar)
{
	...
	float q = 1.0f / std::tan(FrostyMath::DegreesToRadians(0.5f * fov));
...
Matrix4f Matrix4f::Rotate(const Vector3f& eulerAngles)
{
	...
	float x = FrostyMath::DegreesToRadians(eulerAngles.X);
	float y = FrostyMath::DegreesToRadians(eulerAngles.Y);
	float z = FrostyMath::DegreesToRadians(eulerAngles.Z);
